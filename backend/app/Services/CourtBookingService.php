...
class CourtBookingService
{
    ...
    public function lockSlot(array $data)
    {
        ...
        try {
            return DB::transaction(function () use ($courtId, $date, $startTime, $endTime) {
                ...
                CourtBookingRealtimeEvent::dispatch('CourtSlotLocked', [
                    'court_id' => $courtId,
                    'booking_date' => $date,
                    'start_time' => $startTime,
                    'end_time' => $endTime,
    ...
    public function releaseLock(string $lockToken, ?int $userId = null): bool
    {
        return DB::transaction(function () use ($lockToken, $userId) {
            ...
            CourtBookingRealtimeEvent::dispatch('CourtSlotReleased', $payload);
        ...
        try {
            return DB::transaction(function () use ($data, $courtId, $date, $startTime, $endTime) {
                ...
                if ($serviceItems->isNotEmpty()) {
                    $services = CourtService::whereIn('service_id', $serviceItems->pluck('service_id'))
                ...
                $bookingCode = 'BK-'.now()->format('Ymd').'-'.strtoupper(Str::random(4));
                ...
                DB::afterCommit(function () use ($booking) {
                    ...
                    if ($booking->user?->email) {
                        try {
                            ...
                            Log::warning('Failed to queue booking created mail', [
                                'booking_id' => $booking->booking_id,
                                'error' => $e->getMessage(),
                            ]);
                        }
    ...
    {
        $blocks = CourtPrice::where('court_id', $courtId)
            ->where('is_active', true)
