...
class AuthService
{
    ...
    {
        if (strlen($password) < 8) return 'Mật khẩu phải có ít nhất 8 ký tự!';
        if (!preg_match('/[A-Z]/', $password)) return 'Mật khẩu phải chứa ít nhất 1 chữ hoa!';
        if (!preg_match('/[0-9]/', $password)) return 'Mật khẩu phải chứa ít nhất 1 chữ số!';
        if (!preg_match('/[^A-Za-z0-9]/', $password)) return 'Mật khẩu phải chứa ít nhất 1 ký tự đặc biệt!';
        return null;
    }
