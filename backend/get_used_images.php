...
// 6. Admins
if (Illuminate\Support\Facades\Schema::hasColumn('admins', 'avatar')) {
...
// 8. Attendances
if (Illuminate\Support\Facades\Schema::hasTable('attendances') && Illuminate\Support\Facades\Schema::hasColumn('attendances', 'check_out_image_path')) {
    $attendances = DB::table('attendances')->pluck('check_out_image_path');
    foreach ($attendances as $img) addPath($usedImages, $img);
}

// 9. Product Comments (JSON)
if (Illuminate\Support\Facades\Schema::hasTable('product_comments') && Illuminate\Support\Facades\Schema::hasColumn('product_comments', 'images')) {
    $comments = DB::table('product_comments')->pluck('images');
    foreach ($comments as $imagesJson) {
        if ($imagesJson) {
            $images = json_decode($imagesJson, true);
            if (is_array($images)) {
                foreach ($images as $img) addPath($usedImages, $img);
            }
        }
    }
}
...
if (Illuminate\Support\Facades\Schema::hasTable('return_requests') && Illuminate\Support\Facades\Schema::hasColumn('return_requests', 'images')) {
...
// 11. Face Encodings
if (Illuminate\Support\Facades\Schema::hasTable('face_encodings') && Illuminate\Support\Facades\Schema::hasColumn('face_encodings', 'image_path')) {
    $faceEncodings = DB::table('face_encodings')->pluck('image_path');
    foreach ($faceEncodings as $img) addPath($usedImages, $img);
}
