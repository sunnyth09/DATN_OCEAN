const fs = require('fs');
const filePath = 'frontend/src/components/AdminAside.vue';
...
content = content.replace(/const userRoleRaw = ref\(''\);[\s\S]*?const handleSubmenuClick = \(menu\) => \{[\s\S]*?\};\n/, newVars.trim() + '\n\n');
...
content = content.replace(/<nav class="sidebar-nav">[\s\S]*?<\/nav>/, newNav);
